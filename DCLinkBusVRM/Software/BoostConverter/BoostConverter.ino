...
void OnCanMessage(const CANMessage& msg) {
    switch (msg.code) {
        case MessageCode::EMERGENCY_STOP:
            ...
            canBus.SendBool(MessageCode::EMERGENCY_STOP, true); // Acknowledge
            break;

        case MessageCode::REGULAR_STOP:
            OutputEnabled = false;
            canBus.SendBool(MessageCode::REGULAR_STOP, true); // Acknowledge
            break;

        case MessageCode::ENABLE_OUTPUT:
            OutputEnabled = true;
            canBus.SendBool(MessageCode::ENABLE_OUTPUT, true); // Acknowledge
            break;

        case MessageCode::RESET_FAULTS:
            faultManager.resetFaults();
            canBus.SendBool(MessageCode::RESET_FAULTS, true); // Acknowledge
            break;

        case MessageCode::GET_FAULT_COUNT:
            canBus.SendInt(MessageCode::GET_FAULT_COUNT, faultManager.getFaultCount());
            break;
        ...
        case MessageCode::GET_VIN_VOLTAGE:
            canBus.SendFloat(MessageCode::GET_VIN_VOLTAGE, VIN);
            break;

        case MessageCode::GET_VOUT_VOLTAGE:
            canBus.SendFloat(MessageCode::GET_VOUT_VOLTAGE, VOUT);
            break;

        case MessageCode::GET_PHASE1_CURRENT:
            canBus.SendFloat(MessageCode::GET_PHASE1_CURRENT, Phase1Current_A);
            break;

        case MessageCode::GET_PHASE2_CURRENT:
            canBus.SendFloat(MessageCode::GET_PHASE2_CURRENT, Phase2Current_A);
            break;

        case MessageCode::GET_PHASE1_TEMP:
            canBus.SendFloat(MessageCode::GET_PHASE1_TEMP, Phase1Temp_C);
            break;

        case MessageCode::GET_PHASE2_TEMP:
            canBus.SendFloat(MessageCode::GET_PHASE2_TEMP, Phase2Temp_C);
            break;

        case MessageCode::GET_FAN_AIRFLOW:
            canBus.SendInt(MessageCode::GET_FAN_AIRFLOW, SystemFan.getAirflowLFM());
            break;

        case MessageCode::GET_FAN_RPM:
            canBus.SendInt(MessageCode::GET_FAN_RPM, SystemFan.getRPM());
            break;

        case MessageCode::GET_POWER:
            canBus.SendFloat(MessageCode::GET_POWER, VIN * (Phase1Current_A + Phase2Current_A));
            break;

        case MessageCode::GET_CAN_FAULT_STATUS:
            canBus.SendBool(MessageCode::GET_CAN_FAULT_STATUS, canBus.getError());
            break;

        case MessageCode::ENABLE_HEARTBEAT:
            HeartbeatOutput = true;
            canBus.SendBool(MessageCode::ENABLE_HEARTBEAT, true); // Acknowledge
            break;

        case MessageCode::HEARTBEAT_PING:
            lastKeepAliveTime = millis();
            canBus.SendBool(MessageCode::HEARTBEAT_PING, true); // Acknowledge
            break;

        default:
            canBus.SendBool(msg.code, false); // Error response
            break;
    }
}
